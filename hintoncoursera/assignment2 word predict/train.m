...
for epoch = 1:epochs
  ...
  for m = 1:numbatches
    ...
    error_deriv = output_layer_state - expanded_target_batch;%ppt4 p17 softmax dC/dz=yi-ti

    % MEASURE LOSS FUNCTION.
    CE = -sum(sum(...
      expanded_target_batch .* log(output_layer_state + tiny))) / batchsize;%log(0)=-Inf,log(tiny)=-30
    count =  count + 1;
    ...
    % BACK PROPAGATE.
    %% OUTPUT LAYER.
    hid_to_output_weights_gradient =  hidden_layer_state * error_deriv';%dE/dw_ij =yi*  dC/dz( (yj-tj) )  ppt3 p28
    output_bias_gradient = sum(error_deriv, 2);
    back_propagated_deriv_1 = (hid_to_output_weights * error_deriv) ...%dE/dy_i=sum(w_ij*dE/dz_j)
      .* hidden_layer_state .* (1 - hidden_layer_state);
	  % here is logistic error function, not softmax
